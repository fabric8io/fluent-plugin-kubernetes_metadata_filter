...
module KubernetesMetadata
  module CacheStrategy
    def obtain_namespace_metadata(namespace_id, namespace_name)
      @namespace_cache.fetch(namespace_id) do
        @stats.bump(:pod_cache_miss)
        fetch_namespace_metadata(namespace_name)
      end
    end

    def obtain_pod_metadata(pod_name, namespace_name, pod_id)
      @cache.fetch(pod_id) do
